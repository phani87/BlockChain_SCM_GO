...
func (t *PtntVstTraceChaincode) initVisit(stub shim.ChaincodeStubInterface, args []string) pb.Response {
	...
	indexName := "doctorId~visitId"
	ownersIndex := "patientId~visitId"
	err = t.createIndex(stub, indexName, []string{visit.DoctorId, visit.VisitId})
	if err != nil {
		return shim.Error(err.Error())
	}
	err = t.createIndex(stub, ownersIndex, []string{visit.PatientId, visit.VisitId})
	if err != nil {
		return shim.Error(err.Error())
	}
...
// func (t *PtntVstTraceChaincode) transferMobile(stub shim.ChaincodeStubInterface, args []string) pb.Response {
// 	//   0       1       3
// 	// "name", "from", "to"
// 	if len(args) < 3 {
...
// ===========================================================
// func (t *PtntVstTraceChaincode) trannsferMobileHelper(stub shim.ChaincodeStubInterface, IMEINumber string, currentOwner string, newOwner string) (string, error) {
// 	// attempt to get the current vehicle object by serial number.
// 	// if sucessful, returns us a byte array we can then us JSON.parse to unmarshal
